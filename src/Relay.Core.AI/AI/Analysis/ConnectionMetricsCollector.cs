...
internal sealed class ConnectionMetricsCollector
{
        ...
        Func<int> getFallbackConnectionCount)
    {
        try
        {
            ...
            var finalCount = Math.Max(0, connectionCount);

            // Update peak metrics with the final clamped count
            UpdatePeakConnectionMetrics(finalCount);
    ...
    public int GetDatabaseConnectionCount()
    {
        try
        {
            ...
            return Math.Max(0, Math.Min(dbConnections, 100));
        }
        catch (Exception ex)
        {
    ...
    public int GetExternalServiceConnectionCount()
    {
        try
        {
            ...
            return Math.Max(0, Math.Min(externalConnections, 50));
        }
        catch (Exception ex)
        {
    ...
    public int GetWebSocketConnectionCount()
    {
        try
        {
            ...
            return Math.Max(0, Math.Min(wsConnections, _options.MaxEstimatedWebSocketConnections));
        }
        catch (Exception ex)
        {
