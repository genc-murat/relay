...
public static class TransactionServiceCollectionExtensions
{
        ...
        IConfigurationSection configurationSection)
    {
        ...
        // Register and bind options from configuration, with manual handling for TimeSpan conversion
        services.Configure<TransactionOptions>(options =>
        {
            configurationSection.Bind(options);
            
            // If DefaultTimeoutSeconds is configured, convert it to TimeSpan
            var timeoutSecondsValue = configurationSection["DefaultTimeoutSeconds"];
            if (int.TryParse(timeoutSecondsValue, out var timeoutSeconds))
            {
                options.DefaultTimeout = TimeSpan.FromSeconds(timeoutSeconds);
            }
        });
