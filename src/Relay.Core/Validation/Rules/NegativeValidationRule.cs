...
{
    /// <summary>
    /// Validation rule that checks if a numeric value is negative (less than zero).
    /// </summary>
    /// <typeparam name="T">The numeric type to validate.</typeparam>
    public class NegativeValidationRule<T> : IValidationRule<T> where T : struct, IComparable<T>
    {
        /// <inheritdoc />
        public ValueTask<IEnumerable<string>> ValidateAsync(T request, CancellationToken cancellationToken = default)
        {
            ...
            {
                return new ValueTask<IEnumerable<string>>(new[] { "Value must be negative." });
            }
