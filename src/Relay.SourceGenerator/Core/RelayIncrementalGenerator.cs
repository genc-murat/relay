...
public class RelayIncrementalGenerator : IIncrementalGenerator
{
    ...
    {
        // Destructure outside try block so configuration is available in catch
        ...
        catch (Exception ex)
        {
            ...
            // Always report the error diagnostic
            reporter.ReportDiagnostic(
                Diagnostic.Create(
            ...
            if (FallbackGenerator.ShouldUseFallback(ex))
            {
                // Generate complete fallback implementation
                var fallbackOptions = new Generators.GenerationOptions
                {
                    EnableNullableContext = true,
                    IncludeDocumentation = true,
                    EnableDIGeneration = true,
                    EnableOptimizedDispatcher = options.EnableOptimizedDispatcher
                };

                var fallbackSources = FallbackGenerator.GenerateCompleteFallback(fallbackOptions, reporter);
                foreach (var kvp in fallbackSources)
                {
    ...
    {
        var reporter = new SourceOutputDiagnosticReporter(context);
        var options = new Generators.GenerationOptions
        {
            EnableNullableContext = true,
            IncludeDocumentation = true,
            EnableDIGeneration = true
        ...
        FallbackGenerator.ReportFallbackUsage(reporter, "No handlers found in the project");
    ...
    private static void GenerateOptimizedDispatchers(SourceProductionContext context, List<HandlerClassInfo?> handlerClasses, Generators.GenerationOptions options)
    {
        ...
        catch (Exception ex)
        {
            ...
            // Use fallback dispatcher if generation fails
            if (FallbackGenerator.ShouldUseFallback(ex))
            {
                FallbackGenerator.ReportFallbackUsage(reporter, $"Optimized dispatcher generation failed: {ex.Message}");
                var fallbackDispatcher = FallbackGenerator.GenerateFallbackDispatcher(options, ex.Message);
                context.AddSource("OptimizedRequestDispatcher.g.cs", fallbackDispatcher);
            }
            else
            {
                ...
                reporter.ReportDiagnostic(
                    Diagnostic.Create(
                        DiagnosticDescriptors.GeneratorError,
                        Location.None,
                        $"Optimized dispatcher generation critical error: {ex.Message}"));
            }
        }
    }
