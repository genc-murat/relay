...
public class AICircuitBreakerIntegrationTests : IDisposable
{
    ...
    public async Task ExecuteAsync_Timeout_At_Exact_Boundary()
    {
        ...
        {
            await Task.Delay(timeout - TimeSpan.FromMilliseconds(50), ct);
            return "boundary";
        };
    ...
    public async Task Metrics_ResponseTimeStatistics_CalculateCorrectly()
    {
        ...
        Assert.True(Math.Abs(expectedAverage - metrics.AverageResponseTimeMs) < 10, $"Average expected {expectedAverage}, got {metrics.AverageResponseTimeMs}");
        Assert.True(Math.Abs(expected95thPercentile - metrics.Percentile95ResponseTimeMs) < 15, $"95th percentile expected {expected95thPercentile}, got {metrics.Percentile95ResponseTimeMs}");
    }
