...
public class DecisionBoundaryOptimizerTests
{
    ...
    public void UpdatePatterns_Should_Handle_Exception_And_Return_Zero()
    {
        ...
        _loggerMock.Verify(
            x => x.Log(
                LogLevel.Warning,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Error optimizing decision boundaries")),
                It.IsAny<Exception>(),
    ...
    [Fact]
    public void UpdatePatterns_Should_Handle_Empty_ExecutionTimeThresholds()
    {
        // Arrange
        _config.ExecutionTimeThresholds = Array.Empty<int>();
        var predictions = new[]
        {
    ...
    [Fact]
    public void UpdatePatterns_Should_Handle_Negative_Thresholds()
    {
        // Arrange
        _config.ExecutionTimeThresholds = new[] { -50, -100 };
        var predictions = new[]
        {
            CreatePredictionResult(150, TimeSpan.FromMilliseconds(50)), // Since > -50 is true, predicted needs opt, actual does
            CreatePredictionResult(80, TimeSpan.Zero), // > -50 true, but actual no opt needed -> FP
            CreatePredictionResult(120, TimeSpan.FromMilliseconds(30)) // TP
        };
        var analysis = new PatternAnalysisResult();
        ...
        // Assert
        Assert.Equal(1, result);
        Assert.Single(_config.ExecutionTimeThresholds); // Updated to optimal
    }
    ...
    public void UpdatePatterns_Should_Handle_Multiple_Thresholds_And_Select_Best()
    {
        ...
        Assert.Equal(1, result);
        Assert.Single(_config.ExecutionTimeThresholds); // Should be updated to single optimal threshold
        Assert.Equal(50, _config.ExecutionTimeThresholds[0]); // Expected optimal threshold based on accuracy
    }
    ...
    [Fact]
    public void CalculateThresholdMetrics_Should_Handle_No_Positive_Cases()
    {
        // Arrange - All predictions have no actual improvement (no optimization needed)
        var predictions = new[]
        {
            CreatePredictionResult(50, TimeSpan.Zero),  // TN
            CreatePredictionResult(80, TimeSpan.Zero),  // TN
            CreatePredictionResult(25, TimeSpan.Zero)   // TN
        };
        ...
        Assert.Equal(0, thresholdMetrics.TruePositives);
        Assert.Equal(0, thresholdMetrics.FalsePositives);
        Assert.Equal(3, thresholdMetrics.TrueNegatives);
        Assert.Equal(0, thresholdMetrics.FalseNegatives);
        Assert.Equal(0.0, thresholdMetrics.Sensitivity); // When no positives, sensitivity is 0
        Assert.Equal(1.0, thresholdMetrics.Specificity); // 3/(3+0)
        Assert.Equal(0.0, thresholdMetrics.Precision); // 0/(0+0) = 0
        Assert.Equal(0.0, thresholdMetrics.Recall); // Same as sensitivity
        Assert.Equal(1.0, thresholdMetrics.Accuracy); // (0+3)/3
    }
