...
namespace Relay.Core.Tests.AI.Analysis.Engines
{
    public class LoadBasedPatternUpdaterTests
    {
        ...
        public void UpdatePatterns_Should_Calculate_Success_Rates_Per_Load_Level()
        {
            ...
            // Verify debug logging for each load level with success rates
            _loggerMock.Verify(
                x => x.Log(
                    LogLevel.Debug,
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("High") && v.ToString()!.Contains("50.00 %")),
                    It.IsAny<Exception>(),
            ...
            _loggerMock.Verify(
                x => x.Log(
                    LogLevel.Debug,
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Medium") && v.ToString()!.Contains("50.00 %")),
                    It.IsAny<Exception>(),
                    It.IsAny<Func<It.IsAnyType, Exception, string>>()),
                Times.Once);
            ...
            _loggerMock.Verify(
                x => x.Log(
                    LogLevel.Debug,
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Low") && v.ToString()!.Contains("50.00 %")),
                    It.IsAny<Exception>(),
                    It.IsAny<Func<It.IsAnyType, Exception, string>>()),
                Times.Once);
        }
