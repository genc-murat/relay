...
public class RequestTypePatternUpdaterTests
{
    ...
    public void UpdatePatterns_Should_Handle_Empty_Predictions()
    {
        ...
        // Verify info logging for zero request types
        _loggerMock.Verify(
            x => x.Log(
                LogLevel.Information,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Updated patterns for 0 request types")),
                It.IsAny<Exception>(),
                It.IsAny<Func<It.IsAnyType, Exception, string>>()),
            Times.Once);
    }
    ...
    [Fact]
    public void UpdatePatterns_Should_Handle_Null_RequestType_In_Predictions()
    {
        // Arrange
        var predictions = new[]
        {
            CreatePredictionResult(null!, TimeSpan.FromMilliseconds(50))
        };
        var analysis = new PatternAnalysisResult();
    ...
    [Fact]
    public void UpdatePatterns_Should_Handle_Low_Weight_Update_Alpha()
    {
        // Arrange
        _config.WeightUpdateAlpha = 0.1;
        var predictions = new[]
        {
            CreatePredictionResult(typeof(string), TimeSpan.FromMilliseconds(50)),
            CreatePredictionResult(typeof(string), TimeSpan.Zero)
        ...
        // Verify weight calculation: 1.0 * (1 - 0.1) + 0.5 * 0.1 = 0.9 + 0.05 = 0.95
        _loggerMock.Verify(
            x => x.Log(
                LogLevel.Debug,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Weight=0.95")),
                It.IsAny<Exception>(),
                It.IsAny<Func<It.IsAnyType, Exception, string>>()),
            Times.Once);
    }
    ...
    [Fact]
    public void UpdatePatterns_Should_Handle_High_Weight_Update_Alpha()
    {
        // Arrange
        _config.WeightUpdateAlpha = 0.7;
        var predictions = new[]
        {
            CreatePredictionResult(typeof(string), TimeSpan.FromMilliseconds(50)),
            CreatePredictionResult(typeof(string), TimeSpan.Zero)
        ...
        // Verify weight calculation: 1.0 * (1 - 0.7) + 0.5 * 0.7 = 0.3 + 0.35 = 0.65
        _loggerMock.Verify(
            x => x.Log(
                LogLevel.Debug,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Weight=0.65")),
                It.IsAny<Exception>(),
                It.IsAny<Func<It.IsAnyType, Exception, string>>()),
            Times.Once);
    }
    ...
    [Fact]
    public void UpdatePatterns_Should_Handle_Large_Prediction_Arrays()
    {
        // Arrange
        var predictions = new List<PredictionResult>();
        for (int i = 0; i < 500; i++)
        {
            predictions.Add(CreatePredictionResult(typeof(string), TimeSpan.FromMilliseconds(i % 2 == 0 ? 50 : 0)));
            predictions.Add(CreatePredictionResult(typeof(int), TimeSpan.FromMilliseconds(i % 2 == 0 ? 30 : 0)));
        }

        var analysis = new PatternAnalysisResult();

        // Act
        var result = _updater.UpdatePatterns(predictions.ToArray(), analysis);

        // Assert
        Assert.Equal(2, result); // Two distinct request types
    }
