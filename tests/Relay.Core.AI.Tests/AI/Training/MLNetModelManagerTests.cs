...
public class MLNetModelManagerTests : IDisposable
{
    ...
    public void ForecastMetric_WithoutTraining_ReturnsNull()
    {
        ...
        // Act
        var forecast = _manager.ForecastMetric();
    ...
    [Fact]
    public void ForecastMetric_AfterTraining_ReturnsValidForecast()
    {
        // Arrange - Use more data points for better SSA forecasting
        _manager = new MLNetModelManager(_mockLogger.Object, _testModelPath);
        var trainingData = CreateSampleMetricData(200); // Increased data size for SSA
        _manager.TrainForecastingModel(trainingData, horizon: 12);
        ...
        Assert.Equal(12, forecast.LowerBound.Length);
        Assert.Equal(12, forecast.UpperBound.Length);

        // Verify all values are finite (not NaN or Infinity)
        foreach (var value in forecast.ForecastedValues)
        {
            Assert.True(float.IsFinite(value), "Forecasted value should be finite");
        }
        foreach (var value in forecast.LowerBound)
        {
            Assert.True(float.IsFinite(value), "Lower bound should be finite");
        }
        foreach (var value in forecast.UpperBound)
        {
            Assert.True(float.IsFinite(value), "Upper bound should be finite");
        }
    }

    [Fact]
    public void ForecastMetric_ReturnsCorrectHorizon()
    {
        // Arrange
        _manager = new MLNetModelManager(_mockLogger.Object, _testModelPath);
        var trainingData = CreateSampleMetricData(200);

        // Test different trained horizons
        _manager.TrainForecastingModel(trainingData, horizon: 6);
        var forecast6 = _manager.ForecastMetric();
        Assert.NotNull(forecast6);
        Assert.Equal(6, forecast6.ForecastedValues.Length);

        // Retrain with different horizon
        _manager.TrainForecastingModel(trainingData, horizon: 12);
        var forecast12 = _manager.ForecastMetric();
        Assert.NotNull(forecast12);
        Assert.Equal(12, forecast12.ForecastedValues.Length);

        // Retrain with different horizon
        _manager.TrainForecastingModel(trainingData, horizon: 24);
        var forecast24 = _manager.ForecastMetric();
        Assert.NotNull(forecast24);
        Assert.Equal(24, forecast24.ForecastedValues.Length);
    }

    [Fact]
    public void ForecastMetric_WithInsufficientData_ReturnsNull()
    {
        // Arrange
        _manager = new MLNetModelManager(_mockLogger.Object, _testModelPath);
        var trainingData = CreateSampleMetricData(50); // Insufficient data
        _manager.TrainForecastingModel(trainingData, horizon: 12);
