...
public class StreamDispatcherBasicTests
{
    ...
    [Fact]
    public async Task StreamDispatcher_DispatchAsync_WithHandlerName_ThrowsHandlerNotFoundException()
    {
        // Arrange
        var serviceProvider = CreateServiceProvider();
        var dispatcher = new StreamDispatcher(serviceProvider);
        ...
        Func<Task> act = async () =>
        {
            await foreach (var item in dispatcher.DispatchAsync(request, "NonExistentHandler", CancellationToken.None))
            {
                // Should not reach here
            }
        };
    ...
    [Fact]
    public async Task StreamDispatcher_DispatchAsync_WithEmptyHandlerName_ThrowsArgumentException()
    {
        // Arrange
        var serviceProvider = CreateServiceProvider();
        var dispatcher = new StreamDispatcher(serviceProvider);
    ...
    [Fact]
    public async Task StreamDispatcher_DispatchAsync_WithNullHandlerName_ThrowsArgumentException()
    {
        // Arrange
        var serviceProvider = CreateServiceProvider();
        var dispatcher = new StreamDispatcher(serviceProvider);
    ...
    [Fact]
    public void StreamDispatcher_Constructor_WithNullServiceProvider_ThrowsArgumentNullException()
    {
        // Act & Assert
        Assert.Throws<ArgumentNullException>(() => new StreamDispatcher(null!));
    }
}