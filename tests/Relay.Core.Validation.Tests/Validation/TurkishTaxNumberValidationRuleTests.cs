...
{
    public class TurkishTaxNumberValidationRuleTests
    {
        [Fact]
        public async Task ValidateAsync_Should_Return_Empty_Errors_When_Value_Is_Valid_Tax_Number()
        {
            // Arrange
            var rule = new TurkishTaxNumberValidationRule();
            var request = "1234567895"; // Example valid tax number
        ...
        [Fact]
        public async Task ValidateAsync_Should_Return_Error_When_Value_Is_Invalid_Tax_Number()
        {
            // Arrange
            var rule = new TurkishTaxNumberValidationRule();
            var request = "1234567891"; // Invalid checksum
            ...
            Assert.Equal("Invalid Turkish tax number.", errors.First());
        }
        ...
        {
            // Arrange
            var rule = new TurkishTaxNumberValidationRule("Custom tax number error");
            var request = "1234567891";
            ...
            // Assert
            Assert.Single(errors);
            Assert.Equal("Custom tax number error", errors.First());
        }
