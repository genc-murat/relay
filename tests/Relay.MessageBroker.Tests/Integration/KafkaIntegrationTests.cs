...
/// <summary>
/// Integration tests for Kafka message broker functionality
/// </summary>
...
public class KafkaIntegrationTests : IDisposable
{
    ...
    private IMessageBroker CreateBroker()
    {
        ...
        // In a real scenario, this would be the actual KafkaMessageBroker
    ...
    [Fact]
    public async Task PublishAsync_WithPartitionKey_ShouldRouteToPartition()
    {
        // Arrange
        var broker = CreateBroker();
        await broker.StartAsync(CancellationToken.None);

        var message = new TestMessage { Id = "partition-test", Content = "Partitioned message" };
        var publishOptions = new PublishOptions
        {
            Headers = new Dictionary<string, object>
            {
