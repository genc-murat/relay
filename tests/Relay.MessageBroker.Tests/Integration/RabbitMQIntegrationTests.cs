...
/// <summary>
/// Integration tests for RabbitMQ message broker functionality
/// </summary>
...
public class RabbitMQIntegrationTests : IDisposable
{
    ...
    private IMessageBroker CreateBroker()
    {
        ...
        // In a real scenario, this would be the actual RabbitMQMessageBroker
    ...
    public async Task PublishAsync_WithRoutingKey_ShouldUseRoutingKey()
    {
        ...
        var publishOptions = new PublishOptions
        {
            RoutingKey = "custom.queue.name",
            Headers = new Dictionary<string, object>
            {
                ["TargetQueue"] = "custom.queue.name"
            }
        };
    ...
    [Fact]
    public async Task PublishAsync_WithExchangeAndRoutingKey_ShouldRouteCorrectly()
    {
        // Arrange
        var broker = CreateBroker();
        await broker.StartAsync(CancellationToken.None);

        var message = new TestMessage { Id = "exchange-test", Content = "Exchange routing message" };
        var publishOptions = new PublishOptions
        {
    ...
    [Fact]
    public async Task PublishAsync_WithPriority_ShouldHandlePriority()
    {
        // Arrange
        var broker = CreateBroker();
        await broker.StartAsync(CancellationToken.None);

        var message = new TestMessage { Id = "priority-test", Content = "Priority message" };
        var publishOptions = new PublishOptions
        {
            Headers = new Dictionary<string, object>
            {
