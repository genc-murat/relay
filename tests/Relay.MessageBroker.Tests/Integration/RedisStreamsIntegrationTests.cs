...
/// <summary>
/// Integration tests for Redis Streams message broker functionality
/// </summary>
[Trait("Category", "RedisStreams")]
public class RedisStreamsIntegrationTests : IDisposable
{
    ...
    public RedisStreamsIntegrationTests(ITestOutputHelper output)
    {
        _output = output;
    ...
    private IMessageBroker CreateBroker()
    {
        ...
        // In a real scenario, this would be the actual RedisStreamsMessageBroker
    ...
    [Fact]
    public async Task PublishAsync_WithStreamId_ShouldTargetSpecificStream()
    {
        // Arrange
        var broker = CreateBroker();
        await broker.StartAsync(CancellationToken.None);

        var message = new TestMessage { Id = "stream-test", Content = "Stream-specific message" };
        var publishOptions = new PublishOptions
        {
            RoutingKey = "orders-stream",
            Headers = new Dictionary<string, object>
            {
    ...
    [Fact]
    public async Task PublishAsync_WithConsumerGroup_ShouldHandleGroupConsumption()
    {
        // Arrange
        var broker = CreateBroker();
        await broker.StartAsync(CancellationToken.None);

        var message = new TestMessage { Id = "group-test", Content = "Consumer group message" };
        var publishOptions = new PublishOptions
        {
            Headers = new Dictionary<string, object>
            {
