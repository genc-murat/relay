...
public class HandlerRegistryGeneratorTests
{
    ...
    public void CanGenerate_ReturnsTrue_WhenHasHandlers()
    {
        ...
        var discoveryResult = new HandlerDiscoveryResult();
        discoveryResult.Handlers.Add(new HandlerInfo()); // Add a handler
    ...
    [Fact]
    public void GenerateHandlerRegistry_WithTaskNonGenericReturnType_SetsResponseTypeToNull()
    {
        // Arrange
        var compilation = CreateCompilation(@"
                using System.Threading.Tasks;
                namespace Test
                {
                    ...
                    {
                        [Relay.Core.Handle]
                        public Task HandleAsync(ProcessRequest request) => Task.CompletedTask;
                    }
                }");
        ...
        // Assert
        Assert.DoesNotContain("ResponseType =", result); // No response type for Task (non-generic)
    }

    [Fact]
    public void GenerateHandlerRegistry_WithValueTaskNonGenericReturnType_SetsResponseTypeToNull()
    {
        // Arrange
        var compilation = CreateCompilation(@"
                using System.Threading.Tasks;
                namespace Test
                {
                    ...
                    {
                        [Relay.Core.Handle]
                        public ValueTask HandleAsync(ProcessRequest request) => ValueTask.CompletedTask;
                    }
                }");
        ...
        // Assert
        Assert.DoesNotContain("ResponseType =", result); // No response type for ValueTask (non-generic)
    }

    [Fact]
    public void HandlerRegistryGenerator_Properties_ReturnCorrectValues()
    {
        // Arrange
        var compilation = CreateCompilation("");
        var context = new RelayCompilationContext(compilation, default);
        ...
        // Act & Assert
        Assert.Equal("Handler Registry Generator", generator.GeneratorName);
        Assert.Equal("HandlerRegistry", generator.OutputFileName);
        Assert.Equal(20, generator.Priority);
    }
