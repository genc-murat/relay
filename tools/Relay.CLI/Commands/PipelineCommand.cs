...
public static class PipelineCommand
{
    ...
    internal static void DisplayPipelineHeader()
    {
    ...
    internal static async Task<PipelineStageResult> RunInitStage(
        string path,
        string projectName,
        string template,
        bool ciMode,
    ...
    internal static async Task<PipelineStageResult> RunDoctorStage(
        string path,
        bool autoFix,
        bool ciMode,
        CancellationToken cancellationToken)
    ...
    internal static async Task<PipelineStageResult> RunValidateStage(
        string path,
        bool ciMode,
        CancellationToken cancellationToken)
    {
    ...
    internal static async Task<PipelineStageResult> RunOptimizeStage(
        string path,
        bool aggressive,
        bool ciMode,
        CancellationToken cancellationToken)
    ...
    internal static void DisplayPipelineResults(PipelineResult result, bool ciMode)
    {
        if (ciMode)
        {
...
[yellow]Success Rate:[/] {(result.Stages.Count(s => s.Success) * 100.0 / result.Stages.Count):F1}%")
    ...
    internal static async Task GeneratePipelineReport(PipelineResult result, string reportPath)
    {
...
- **Success Rate:** {(result.Stages.Count == 0 ? 0.0 : (result.Stages.Count(s => s.Success) * 100.0 / result.Stages.Count)):F1}%
